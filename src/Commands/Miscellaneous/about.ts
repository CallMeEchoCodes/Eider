...
const About: Command = {
  ...
  async run (Client: Bot, Interaction: CommandInteraction): Promise<void> {
    const messageCount = await Client.database.selectColumnAll('overallData', 'messageCount')
    const commandCount = await Client.database.selectColumnAll('overallData', 'commandCount')
    ...
    const embed = new MessageEmbed()
      ...
      .addField('Messages Recieved', `${messageCount[0].messageCount}`, true)
      .addField('Commands Recieved', `${commandCount[0].commandCount}`, true)
