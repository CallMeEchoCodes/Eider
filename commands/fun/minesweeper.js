...
module.exports = {
	...
	execute(client, message, args) {
		...
		function generateGame(gameWidth, gameHeight, numMines, message, startsNotUncovered, isRaw) {
			...
			// Initialize vars
			const zeroLocations = []; // Array of {x,y} objects, will contain locations of all zeroes in the game
			const uncoveredLocations = []; // 2D array, each value is either nothing (not uncovered) or true (uncovered)
			...
			if (startsNotUncovered) {
				...
				if (zeroLocations.length > 0) {
					...
					const firstCoord = zeroLocations[Math.floor(Math.random() * zeroLocations.length)];
